  ...
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="exampleModalLabel">이용하신 객실을 선택해 주세요</h5>
      ...
      <div class="modal-body">
        ...
        <c:forEach items="${sublist }" var="s">
        	<p>${payment_orderdate } ${private String room_name }</p>
        </c:forEach>
        
      </div>
      <div class="modal-footer">
        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">취소</button>
        <button type="button" class="btn btn-primary" onclick="location.href='review_write.do?id=<%=id %>&page=${paging.page }'">리뷰 작성하기</button>
      </div>
    </div>
  </div>
</div>
                ...
                <div id="table_wrap">
					...
					<th>조회수</th>
                        ...
                        <c:if test="${!empty list }">
							...
							<td  class="board_title"><a href="<%=request.getContextPath() %>/review_content.do?no=${r.review_no }&page=${paging.page }">
				               							${r.review_title } </a></td>
							<td>${r.review_id }</td>
